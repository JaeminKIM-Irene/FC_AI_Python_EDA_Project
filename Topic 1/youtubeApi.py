...
class YoutubeApi():
    ...
    # 초기화  
    def __init__(self):
      self.apiKey = os.environ.get('YOUTUBE_KEY')
    ...
    def getDictByVideoId(self, videoId):
        search = f"videos?part=statistics&maxResults=1&key={self.apiKey}&id={parse.quote(videoId)}"
        res = requests.get(self.url + search)
        jsonData = res.text
        dictData = json.loads(jsonData)
